...
const TodoItemStyle = styled.div`
  ...
  `}
`
function DoneButton({ className, children, isDone, onClick }) {
  return (
    <DoneButtonStyle isDone={isDone} className={className} onClick={onClick}>
      {children}
    </DoneButtonStyle>
  )
}
...
function DeleteButton({ className, children, onClick }) {
  return (
    <DeleteButtonStyle className={className} onClick={onClick}>
      {children}
    </DeleteButtonStyle>
  )
}
...
export default function TodoItem({ todo, handleToggle, handleDelete }) {
  ...
  return (
    <TodoDiv data-id={todo.id}>
      ...
      <ButtonDiv>
        <DoneButton isDone={todo.isDone} onClick={handleToggleClick}>
          {todo.isDone ? '已完成' : '未完成'}
        </DoneButton>
